#include "composition/counter_test/preloaded.h"  // IWYU pragma: keep
...
dom_models_error_t cmp_counter_test_preloaded_load_from_nvs(nvs_handle_t nvs) {
    if (!nvs) {
...
#ifdef COMPOSITION_COUNTER_TEST_CONFIG_PRELOADED_WIFI_AP_SSID_USE_DEVICE_ID
    err = apply_wifi_ap_ssid_device_id_suffix();
    if (err != DOMAIN_MODELS_ERROR_OK) {
        clear_preloaded();
        return err;
    }
#endif /* COMPOSITION_COUNTER_TEST_CONFIG_PRELOADED_WIFI_AP_SSID_USE_DEVICE_ID */

    return DOMAIN_MODELS_ERROR_OK;
}

void cmp_counter_test_preloaded_free() {
    clear_preloaded();
}
