...
#include "composition/infrared/preloaded.h"
...
static dom_models_error_t init_nvs(cmp_infrared_launcher_t* launcher);
static dom_models_error_t init_mqtt_client(cmp_infrared_launcher_t* launcher);
static dom_models_error_t init_ble(cmp_infrared_launcher_t* launcher);
...
dom_models_error_t cmp_infrared_driver_init(cmp_infrared_launcher_t* launcher) {
    ...
    err = cmp_infrared_preloaded_load_from_nvs(launcher->driver.nvs);
    if (err != DOMAIN_MODELS_ERROR_OK) {
        return err;
    }
...
static dom_models_error_t init_ble(cmp_infrared_launcher_t* launcher) {
    ...
    char               device_name[40];
    dom_models_error_t err = cmp_infrared_utils_build_ble_device_name(device_name, sizeof(device_name));
    if (err != DOMAIN_MODELS_ERROR_OK) {
        return err;
    }
...
static dom_models_error_t init_mqtt_client(cmp_infrared_launcher_t* launcher) {
    char               client_id[64];
    dom_models_error_t err = cmp_infrared_utils_build_mqtt_client_id(client_id, sizeof(client_id));
    if (err != DOMAIN_MODELS_ERROR_OK) {
        return err;
    }

    char broker_uri[160];
    err = cmp_infrared_utils_build_mqtt_broker_uri(broker_uri, sizeof(broker_uri));
    if (err != DOMAIN_MODELS_ERROR_OK) {
        return err;
    }

    char lwt_topic[64];
    err = cmp_infrared_utils_build_mqtt_lwt_topic(lwt_topic, sizeof(lwt_topic));
    if (err != DOMAIN_MODELS_ERROR_OK) {
        return err;
    }
    ...
    if (cmp_infrared_utils_cstr_available(dom_models_preloaded_data.mqtt_user)) {
        mqtt_cfg.credentials.username = dom_models_preloaded_data.mqtt_user;
        if (cmp_infrared_utils_cstr_available(dom_models_preloaded_data.mqtt_pass)) {
