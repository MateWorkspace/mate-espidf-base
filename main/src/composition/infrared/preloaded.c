#include "composition/infrared/preloaded.h"  // IWYU pragma: keep
...
#ifdef COMPOSITION_INFRARED_CONFIG_PRELOADED_WIFI_AP_SSID_USE_DEVICE_ID
    err = apply_wifi_ap_ssid_device_id_suffix();
    if (err != DOMAIN_MODELS_ERROR_OK) {
        clear_preloaded();
        return err;
    }
#endif /* COMPOSITION_INFRARED_CONFIG_PRELOADED_WIFI_AP_SSID_USE_DEVICE_ID */
