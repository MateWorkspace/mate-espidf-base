...
static dom_models_error_t telemetry_impl(
    ...
    if (!self || !self->ctx || !device_id || device_id[0] == '\0' ||
        ...
        !payload_schema_name || payload_schema_name[0] == '\0' ||
        !payload_json || payload_json[0] == '\0') {
        return DOMAIN_MODELS_ERROR_BAD_ARGUMENT;
    }
    ...
    char               topic[INF_MESSAGING_DEF_PUB_MQTT_IMPL_TOPIC_MAX_LEN];
    dom_models_error_t err = inf_messaging_def_pub_mqtt_impl_build_device_topic(device_id, "telemetry", topic, sizeof(topic));
    if (err != DOMAIN_MODELS_ERROR_OK) {
        return err;
    }
    ...
    return inf_messaging_def_pub_mqtt_impl_publish_json(
        ctx,
        topic,
        inf_messaging_def_pub_mqtt_impl_build_telemetry_json(metric_name, payload_schema_name, payload_schema_version, payload_json),
        INF_MESSAGING_DEF_PUB_MQTT_IMPL_QOS_DEFAULT,
        false
    );
}
