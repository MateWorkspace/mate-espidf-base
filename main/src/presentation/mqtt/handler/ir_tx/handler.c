...
void pres_mqtt_handler_ir_tx(pres_mqtt_context_t* ctx, const char* data, int data_len) {
    ...
    pres_mqtt_handler_ir_tx_dto_request_t* request = &ctx->ir_tx_request_scratch;
    ...
    if (request->raw_data_lossy) {
        ctx->logger->error(ctx->logger, tag, "Invalid ir/tx payload: malformed or oversized raw_data payload");
        (void)ctx->def_pub->ir_transmit_ack(ctx->def_pub, ctx->device_id_str, request->execution_id, IR_TX_ACK_STATUS_FAILED, "malformed or oversized raw_data payload");
        return;
    }
